...
// ---------------- responsivo: escala o palco (1600x1320) para caber na tela ----------------
const STAGE_W = 1600, STAGE_H = 1320, STAGE_MIN_SCALE = 0.5;
function fitStage(){
  ...
  let scale = Math.min(1, available / STAGE_W);
  scale = Math.max(scale, STAGE_MIN_SCALE);
  stage.style.transform = `scale(${scale})`;
  frame.style.width = Math.round(STAGE_W * scale) + 'px';
...
function drawConnectors(){
  ...
  const renderScale = stageRect.width / stage.offsetWidth;
  svg.innerHTML = '';
  function centerOf(el){
    const r = el.getBoundingClientRect();
    return { x: (r.left - stageRect.left + r.width/2) / renderScale, y: (r.top - stageRect.top + r.height/2) / renderScale };
  }
